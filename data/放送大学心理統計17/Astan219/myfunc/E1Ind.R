##########################################################################
#独立な1要因の実験データを分析
#■入力
#y: ベクトル形式の特性値
#A: ベクトル形式の水準(1からaまで、抜けなしで、整数で指定)
#prior=F : 論理値。Tなら事前分布の範囲を指定。Fなら指定せず。
#mL=-1000, mH=1000, sL=0, sH=100 : 事前分布のパラメタ
#prob=c(0.025, 0.05, 0.5, 0.95, 0.975): 事後分布で報告する確率点
#see=1234,cha=5,war=1000,ite=21000 : MCMC関連のパラメタ
#■出力
# fit:stanの出力, par:母数リスト, prob:確率ベクトル, 
# muA:水準の平均, sigmaE:水準内sd, sigmaA:水準間sd, eta2:説明率
# delta:効果量, mu:全平均, aj;水準の効果, Ubig:水準の効果0以上,
# Usma:水準の効果0以下, U2:i行がj列より大きい, log_lik:対数尤度
##########################################################################
E1Ind <- function(y,A,prior=F,mL=-1000, mH=1000, sL=0, sH=100,
prob=c(0.025, 0.05, 0.5, 0.95, 0.975),
see=1234, cha=5, war=1000, ite=21000, fi=NA)
{
   library(rstan)
   if (prior) {
     dat <- list(n=length(y),a=max(A),y=y,A=A,mL=mL,mH=mH,sL=sL,sH=sH)
     scr <- "stan/E1IndPT.stan"                           # Stan ファイル名
   } else {
     dat <- list(n=length(y),a=max(A),y=y,A=A)
     scr <- "stan/E1IndPF.stan"                           # Stan ファイル名
   }
   par<-c("muA","sigmaE","sigmaA","eta2","delta",
       "mu","aj","Ubig","Usma","U2","log_lik")  # サンプリング変数
   fit<-stan(file=scr,data=dat,pars=par,seed=see,chains=cha,warmup=war,iter=ite, fit=fi)
   ext<-extract(fit,par);muA<-ext$muA;sigmaE<-ext$sigmaE;sigmaA<-ext$sigmaA; 
   eta2<-ext$eta2;delta<-ext$delta;mu<-ext$mu;aj<-ext$aj;Ubig<-ext$Ubig;
   Usma<-ext$Usma;U2<-ext$U2;log_lik<-ext$log_lik;
   out<-list(prior=prior,fit=fit,par=par,prob=prob,muA=muA,
      sigmaE=sigmaE,sigmaA=sigmaA,eta2=eta2,delta=delta,mu=mu,aj=aj,
      Ubig=Ubig,Usma=Usma,U2=U2,log_lik=log_lik)
   class(out)<-'E1Ind'
   return(invisible(out))
}
##########################################################################
#印刷のメソッド
#■入力
#x:クラス 'E1Ind'のオブジェクト
#degits=3 : 小数の丸め
##########################################################################
print.E1Ind<-function(x,degits=3)
{
   prior<-x$prior;log_lik<-x$log_lik;
   if (prior) {print("******************事前分布指定******************")}
         else {print("******************事前分布デフォルト************")}
   waic<- (-2)*(log(mean(exp(log_lik)))) + 2*(var(log_lik))
   print(x$fit,pars=x$par,digits_summary=degits,probs=x$prob)
   print(paste("waic=",round(waic,degits),sep=""))
   out<-list(waic=waic)
   return(invisible(out))
}  
##########################################################################
#連言命題が成り立つ確率(E1IndとE2Indで共通の関数)
#■入力
#x:クラス U2[,i,j]iがjより大きい1そうでないとき0 あるいはU2A,U2B
#degits=3 : 小数の丸め
#IJ : 2列の行列（1列目は行、2列目は列）
##########################################################################
printIJ<-function(x,degits=3,IJ)
{
   pro<-1
   for (i in 1:nrow(IJ)){
      pro<-pro*x[,IJ[i,1],IJ[i,2]]
   }
   print(round(mean(pro),degits))
}
##########################################################################
#２つの水準の比較
#■入力
#x:クラス 'E1Ind'のオブジェクト
#degits=3 : 小数の丸め
#I: 整数、平均値の大きい水準
#J: 整数、平均値の小さい水準
#cr1: 閾上率の基準値
##########################################################################
E1betw_level<-function(x,degits=3,I,J,cr1=F)
{
   G<-matrix(0,length(x$mu),5)
   G[,1] <- x$muA[,I]-x$muA[,J];
   G[,2] <- G[,1]/x$sigmaE;
   G[,3] <- pnorm(x$muA[,I],x$muA[,J],x$sigmaE);
   G[,4] <- pnorm((G[,2]/sqrt(2)), 0.0, 1.0);
   lab<-c("平均値の差","効果量","非重複度","優越率")
   co<-4
   if(is.numeric(cr1)){  co<-co+1;
      G[,5] <- pnorm(((G[,1]-cr1)/(sqrt(2)*x$sigmaE)), 0.0, 1.0);
      lab<-c(lab,paste("閾上率(",cr1,")",sep=""));}
   Gc<-cbind(
      apply(G[,1:co],2,mean),
      apply(G[,1:co],2,sd),
      t(apply(G[,1:co],2,quantile, probs=c(0.025, 0.05, 0.5, 0.95, 0.975)))
   )
   colnames(Gc)<-c("EAP","post.sd","2.5%","5%","50%","95%","97.5%")
   rownames(Gc)<-lab
   round(Gc,degits)
}

