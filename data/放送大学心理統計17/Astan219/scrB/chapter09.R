...
out<-E1Ind(y,A,prior=T,mL=0, mH=100, sL=0, sH=50, fi=NA)
print(out,3)

#図9.2
size<-dim(out$mu);              #乱数数
yy<-out$aj[,1]; for (i in 2:4){yy<-c(yy,out$aj[,i])};
boxplot(yy~rep(c("x1対照","x2聴音","x3音読","x4運動"),each=size))
abline(h=0.0,lwd=2.0)

#説明率と効果量の事後分布
hist(out$eta2, breaks=100)       #図9.3
hist(out$delta, breaks=100)      #図9.4

#連言命題が正しい確率
printIJ(out$U2,IJ=rbind(c(3,2),c(2,1),c(1,4)))
printIJ(out$U2,IJ=rbind(c(3,1),c(2,1),c(1,4)))
printIJ(out$U2,IJ=rbind(c(3,4),c(2,4),c(1,4)))

#特に興味のある2水準間の比較
E1betw_level(out,3,3,2,cr1=0.5)
E1betw_level(out,3,3,4,cr1=2.0)
